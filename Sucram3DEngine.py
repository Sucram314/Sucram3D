...
c3 = -1/math.tan(math.radians((((1280/658)*c))/2))
topPlane = (0,1/math.sqrt(1+c3**2),c3/math.sqrt(1+c3**2))
c4 = -1/math.tan(math.radians((((1280/658)*c))/-2))
...
def filltriangle(x1,y1,z1,x2,y2,z2,x3,y3,z3,playerdata,avg,rgb):
    ...
    if clipped != 0: triangles.extend(clipped)

    for i in range(len(triangles)):
        clipped = TrianglePlaneClip(rightPlane,(0,0,0),triangles.pop(0),dpnp2)
        if clipped != 0: triangles.extend(clipped)

    for i in range(len(triangles)):
        clipped = TrianglePlaneClip(leftPlane,(0,0,0),triangles.pop(0),dpnp3)
        if clipped != 0: triangles.extend(clipped)

    for i in range(len(triangles)):
        clipped = TrianglePlaneClip(topPlane,(0,0,0),triangles.pop(0),dpnp4)
        if clipped != 0: triangles.extend(clipped)

    for i in range(len(triangles)):
        clipped = TrianglePlaneClip(bottomPlane,(0,0,0),triangles.pop(0),dpnp5)
        if clipped != 0: triangles.extend(clipped)

    for i in range(len(triangles)):
        clipped = TrianglePlaneClip((0,0,-1),(0,0,-zClip),triangles.pop(0),dpnp1)
        if clipped != 0: triangles.extend(clipped)

    listOfTriangles = []

    for triangle in triangles:
        triangle[0][0] = playerdata[5]*(triangle[0][0]/-triangle[0][2])+hwidth
        triangle[0][1] = playerdata[5]*(triangle[0][1]/-triangle[0][2])+hheight
        triangle[1][0] = playerdata[5]*(triangle[1][0]/-triangle[1][2])+hwidth
        triangle[1][1] = playerdata[5]*(triangle[1][1]/-triangle[1][2])+hheight
        triangle[2][0] = playerdata[5]*(triangle[2][0]/-triangle[2][2])+hwidth
        triangle[2][1] = playerdata[5]*(triangle[2][1]/-triangle[2][2])+hheight
        triangle[0][2] *= extra
        triangle[1][2] *= extra
        triangle[2][2] *= extra

        clipped = TrianglePlaneClip((0,1,0),(0,0,0),triangle,dpnp6)
        if clipped != 0: listOfTriangles.extend(clipped)

        for i in range(len(listOfTriangles)):
            clipped = TrianglePlaneClip((0,-1,0),(0,height,0),listOfTriangles.pop(0),dpnp7)
            if clipped != 0: listOfTriangles.extend(clipped)

        for i in range(len(listOfTriangles)):
            clipped = TrianglePlaneClip((1,0,0),(0,0,0),listOfTriangles.pop(0),dpnp8)
            if clipped != 0: listOfTriangles.extend(clipped)

        for i in range(len(listOfTriangles)):
            clipped = TrianglePlaneClip((-1,0,0),(width,0,0),listOfTriangles.pop(0),dpnp9)
            if clipped != 0: listOfTriangles.extend(clipped)
...
def drawTriangle(triangle):
    pygame.gfxdraw.aapolygon(screen,triangle[0],triangle[1])
    pygame.gfxdraw.filled_polygon(screen,triangle[0],triangle[1])
